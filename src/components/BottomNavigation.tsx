...
export const BottomNavigation: React.FC = () => {
  ...
  return (
    <nav className="items-center flex w-full text-sm text-white font-normal whitespace-nowrap text-center leading-[1.2] bg-black pt-4 pb-6 px-2 border-t-[rgba(113,104,96,0.30)] border-t border-solid">
      {navItems.map((item) => (
        <button
          key={item.id}
        ...
        >
          <img
            ...
            alt=""
          />
          <span className={`mt-2 ${activeTab === item.id ? 'text-white' : 'text-gray-400'}`}>
            {item.label}
          </span>
        </button>
      ))}
